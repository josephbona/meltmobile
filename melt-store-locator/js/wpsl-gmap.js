...
function showStores() {
	...
	var newStart = getUrlZipcode();

	if (newStart != null && newStart != ''){

		geocoder.geocode( { 'address': newStart}, function(results, status) {
	      if (status == google.maps.GeocoderStatus.OK) {
	         lat = results[0].geometry.location.lat();
	         lng = results[0].geometry.location.lng();
	         newLatLng = new google.maps.LatLng( lat, lng );
	         addMarker( newLatLng, 0, startMarker, true ); // This marker is the 'start location' marker. With a storeId of 0, no name and is draggable
	         findStoreLocations( newLatLng, resetMap, autoLoad );
	      } else {
...
/* DL - Get zipcode from GET URL */
function getUrlZipcode(){
	var $_GET = {};
