...
*/
var elementExists = document.getElementById('masthead');
	...
	$( window ).scroll( function( event ) {
		...
		if ( $( window ).width() > 800 ) {
			var position = window.scrollY,
				bottom   = window.innerHeight - document.getElementById( 'footer' ).offsetHeight,
				height   = $( '#masthead' ).data( 'height' ),
				content  = $( '#content' ).offset().top,
				footer   = $( '#footer' ).offset().top - position;
